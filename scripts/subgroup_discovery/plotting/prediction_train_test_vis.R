#####################################################################
## This script generates all subgroup visuals for model prediction ##
#####################################################################
...
# some utilitie objects used for vis loop
metric_order <- c("AUROC", "AVG Precision")
...
# set theme size default for ggplot
...
for(metric in metric_order){
    ...
    prediction_spreadsheet <- paste0("./results/subgroup_discovery/subgroup_",metric,"_prediction_results.xlsx")
    baseline_list <- as.data.frame(read_xlsx(prediction_spreadsheet, "baseline @ 20% Data"))
    baseline_list[,2:5] <- round(baseline_list[,2:5],3)

    #load baseline_rand measurements at 20%
    baseline_rand_list <- as.data.frame(read_xlsx(prediction_spreadsheet,"rand baseline @ 20% Data"))
    baseline_rand_list[,2:5] <- round(baseline_rand_list[,2:5],3)


    for(outcome in outcome_order){
        PR_20 = as.data.frame(read_xlsx(prediction_spreadsheet, paste0(outcome,"-Kfold PR @ 20%")))
        PR_val_20 = as.data.frame(read_xlsx(prediction_spreadsheet, paste0(outcome,"-Val PR @ 20%")))
        ...
        ROC_20 = as.data.frame(read_xlsx(prediction_spreadsheet, paste0(outcome,"-Kfold ROC @ 20%")))
        ROC_val_20 = as.data.frame(read_xlsx(prediction_spreadsheet, paste0(outcome,"-Val ROC @ 20%")))
    ...
    pdf(paste0("./results/subgroup_discovery/prediction_",metric,"kfold_PR_curves.pdf"), height=7, width=7, useDingbats=FALSE)
    print(PR_combined_plot)
    dev.off()

    pdf(paste0("./results/subgroup_discovery/prediction_",metric,"_val_PR_curves.pdf"), height=7, width=7, useDingbats=FALSE)
    ...
    pdf(paste0("./results/subgroup_discovery/prediction_",metric,"_kfold_ROC_curves.pdf"), height=7, width=7, useDingbats=FALSE)
    print(ROC_combined_plot)
    dev.off()

    pdf(paste0("./results/subgroup_discovery/prediction_",metric,"_val_ROC_curves.pdf"), height=7, width=7, useDingbats=FALSE)
