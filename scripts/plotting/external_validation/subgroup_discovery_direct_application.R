...
option_list <- list(
    make_option(
        c("-i", "--input"), default='./results/external_validation/mednax/direct_subgroup_discovery/',
        help="Input directory for direct subgroup discovery application experiments"),
     make_option(
        c("-o", "--output"), default="./results/external_validation/mednax/direct_subgroup_discovery/plots/",
...
for(metric in metric_order){
    #iterate over outcomes to construct kfold + val dataframes
    PR_df <- data.frame()
    ...
    bottleneck_spreadsheet <- paste0(input_dir, metric, "_bottleneck_results.xlsx")
    ...
    baseline_rand_list[,2:5] <- round(baseline_rand_list[,3:5],3)
    ...
    pdf(paste0(output_dir, "bottleneck_", metric,"_test_set_PR_curves.pdf"), height=7, width=7, useDingbats=FALSE)
    print(PR_combined_plot)
    dev.off()

    pdf(paste0(output_dir, "bottleneck_", metric, "_val_PR_curves.pdf"), height=7, width=7, useDingbats=FALSE)
    ...
    pdf(paste0(output_dir, "bottleneck_", metric,"_test_set_ROC_curves.pdf"), height=7, width=7, useDingbats=FALSE)
    print(ROC_combined_plot)
    dev.off()

    pdf(paste0(output_dir, "bottleneck_", metric,"_val_ROC_curves.pdf"), height=7, width=7, useDingbats=FALSE)
